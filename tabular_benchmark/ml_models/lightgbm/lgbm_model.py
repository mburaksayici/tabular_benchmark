...
class LGBMModel:
    ...
    def preprocess(self, df, data_params):
        ...
        dtrain = lgb.Dataset(
            train_x, label=train_y, categorical_feature=object_columns_feat
        )
        dvalid = lgb.Dataset(
            valid_x, label=valid_y, categorical_feature=object_columns_feat
        )
    ...
    def fit_model(self, preprocessed_data, hyperparam_opt=True):
        ...
        if hyperparam_opt:
            ...
            study.optimize(
                OptunaObjective(
                    problem_type="classification", dtrain=dtrain, dvalid=dvalid
                ),
